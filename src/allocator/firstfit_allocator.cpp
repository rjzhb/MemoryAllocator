...
FirstFitAllocator::FirstFitAllocator(int size) {
    //空闲内存大小size
    free_chunk_list_.push_back(MemoryChunk{INVALID, 0, size});
}


bool FirstFitAllocator::alloc(int id, int size) {
    //分配作业id大小size的内存
...
bool FirstFitAllocator::free(int id) {
    //释放作业id的内存
